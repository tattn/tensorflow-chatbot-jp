...
def mecab_tokenizer(sentence):
  """Mecab tokenizer: split the japanese sentence into a list of tokens."""
  return mecab.parse(sentence).split(' ')[:-1]
...
def prepare_dialog_data(data_dir, vocabulary_size):
  ...
  create_vocabulary(vocab_path, train_path + ".in", vocabulary_size, mecab_tokenizer)

  # Create token ids for the training data.
  train_ids_path = train_path + (".ids%d.in" % vocabulary_size)
  data_to_token_ids(train_path + ".in", train_ids_path, vocab_path, mecab_tokenizer)

  # Create token ids for the development data.
  dev_ids_path = dev_path + (".ids%d.in" % vocabulary_size)
  data_to_token_ids(dev_path + ".in", dev_ids_path, vocab_path, mecab_tokenizer)
